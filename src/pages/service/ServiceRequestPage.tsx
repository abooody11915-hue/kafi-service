...
export function ServiceRequestPage() {
  ...
  const canNext = useMemo(() => {
    ...
    if (step === 2) return form.title.trim().length >= 3 && form.description.trim().length >= 5;
  ...
  const create = useMutation({ mutationFn: async () => {
    ...
    const { data, error } = await (supabase as any).rpc("create_maintenance_request_atomic", {
      ...
      service_metadata_input: { service_label: selected.name_ar, family: selected.family_code }, idempotency_key_input: idempotencyKey,
    });
  ...
  }, onSuccess: ({ id, failed }) => { localStorage.removeItem(DRAFT_KEY); setCreatedId(id); if (failed.length) toast.warning(`تم إنشاء الطلب، وتعذر رفع ${failed.length} مرفق`); else toast.success("تم إرسال طلبك بنجاح"); }, onError: (error: Error) => toast.error(error.message) });

  if (createdId) return <ServiceShell><div className="mx-auto max-w-lg py-10 text-center"><div className="mx-auto grid h-20 w-20 place-items-center rounded-[28px] bg-primary/10 text-primary"><Check className="h-9 w-9" /></div><h1 className="mt-5 text-2xl font-black">تم استلام طلبك</h1><p className="mt-2 text-[11px] leading-6 text-muted-foreground">بدأ سجل التتبع، وسيظهر أي عرض أو تحديث فورًا داخل طلباتك.</p><Button onClick={() => navigate("/requests")} className="mt-6 h-12 rounded-xl px-8 font-black">متابعة الطلب</Button></div></ServiceShell>;

  return <ServiceShell compact><div className="mx-auto max-w-2xl">
    <div className="mb-5"><p className="text-[10px] font-black text-primary">طلب خدمة جديد</p><h1 className="text-2xl font-black">{STEPS[step - 1]}</h1><div className="mt-4 grid grid-cols-5 gap-1.5">{STEPS.map((_, index) => <div key={index} className={cn("h-1.5 rounded-full", index < step ? "bg-primary" : "bg-muted")} />)}</div><p className="mt-2 text-[9px] text-muted-foreground">الخطوة {step} من 5</p></div>
    <section className="rounded-[28px] border border-[#e5e2d9] bg-white p-4 shadow-[0_20px_55px_-45px_rgba(5,68,49,.7)] md:p-6">
      ...
      {step === 2 && <div className="space-y-4"><div><label className="text-[11px] font-black">عنوان مختصر</label><Input value={form.title} onChange={(event) => update("title", event.target.value)} maxLength={160} className="mt-2 h-12 rounded-xl text-base" placeholder="مثال: المكيف لا يبرد" /></div><div><label className="text-[11px] font-black">صف المشكلة كما تراها</label><Textarea value={form.description} onChange={(event) => update("description", event.target.value)} maxLength={1500} rows={5} className="mt-2 resize-none rounded-xl text-base" placeholder="متى بدأت؟ ماذا جرّبت؟ وهل يوجد تسرب أو صوت؟" /></div><div><label className="text-[11px] font-black">الأولوية</label><div className="mt-2 grid grid-cols-3 gap-2">{([['normal','عادي'],['urgent','عاجل'],['emergency','طارئ']] as const).map(([value,label]) => <button key={value} onClick={() => update("priority", value)} className={cn("h-11 rounded-xl text-[10px] font-black", form.priority === value ? "bg-primary text-white" : "bg-secondary")}>{label}</button>)}</div></div></div>}
