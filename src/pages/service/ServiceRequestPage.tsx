...
import { LocationPicker, type SelectedLocation } from "@/components/service/LocationPicker";
...
type Form = { serviceCode: string; title: string; description: string; latitude: number | null; longitude: number | null; addressLabel: string; city: string; district: string; street: string; buildingNo: string; unitNo: string; locationNote: string; phone: string; priority: Priority; timing: Timing; date: string; time: string; pricing: Pricing; offeredPrice: string };
const EMPTY: Form = { serviceCode: "", title: "", description: "", latitude: null, longitude: null, addressLabel: "", city: "", district: "", street: "", buildingNo: "", unitNo: "", locationNote: "", phone: "", priority: "normal", timing: "asap", date: "", time: "", pricing: "offers", offeredPrice: "" };
const DRAFT_KEY = "kafi-service.request-draft.v2";
const STEPS = ["الخدمة", "وصف الطلب", "الموقع والموعد", "الدخول الآمن", "المراجعة والمرفقات"];
...
export function ServiceRequestPage() {
  ...
  const canNext = useMemo(() => {
    ...
    if (step === 3) return form.latitude !== null && form.longitude !== null && (form.timing === "asap" || (!!form.date && !!form.time));
  ...
  const validationMessage = useMemo(() => {
    ...
    if (step === 3 && (form.latitude === null || form.longitude === null)) return "حدد موقع الخدمة على الخريطة.";
    if (step === 3 && form.timing === "scheduled" && (!form.date || !form.time)) return "حدد تاريخ الموعد ووقته.";
    return null;
  }, [step, selected, form]);

  const update = <K extends keyof Form>(key: K, value: Form[K]) => setForm((current) => ({ ...current, [key]: value }));
  const updateLocation = (location: SelectedLocation) => setForm((current) => ({
    ...current,
  ...
  const create = useMutation({ mutationFn: async () => {
    ...
    const { data, error } = await (supabase as any).rpc("create_maintenance_request_atomic", {
      ...
      address_input: { city: form.city.trim() || "موقع محدد على الخريطة", district: form.district.trim() || "غير محدد", street: form.street.trim(), building_no: form.buildingNo.trim(), unit_no: form.unitNo.trim(), latitude: form.latitude, longitude: form.longitude, label: form.addressLabel || "موقع الطلب" },
      ...
      service_metadata_input: { service_label: selected.name_ar, family: selected.family_code, location_source: "map" }, idempotency_key_input: idempotencyKey.current,
    });
  ...
  return <ServiceShell compact><div className="mx-auto max-w-2xl">
    ...
    <section className="rounded-[28px] border border-[#e5e2d9] bg-white p-4 shadow-[0_20px_55px_-45px_rgba(5,68,49,.7)] md:p-6">
      ...
      {step === 3 && <div className="space-y-5"><LocationPicker value={form.latitude !== null && form.longitude !== null ? { latitude: form.latitude, longitude: form.longitude, label: form.addressLabel, city: form.city, district: form.district, street: form.street } : null} onChange={updateLocation} /><div><div className="mb-2 flex items-center justify-between"><label className="text-sm font-black">تفاصيل المبنى والشقة</label><span className="text-xs font-bold text-muted-foreground">اختيارية</span></div><div className="grid grid-cols-2 gap-3"><Input value={form.buildingNo} onChange={(event) => update("buildingNo", event.target.value)} placeholder="رقم المبنى" className="h-12 rounded-xl text-base" /><Input value={form.unitNo} onChange={(event) => update("unitNo", event.target.value)} placeholder="رقم الشقة" className="h-12 rounded-xl text-base" /></div></div><div><label className="text-sm font-black">موعد الخدمة</label><div className="mt-2 grid grid-cols-2 gap-2"><button onClick={() => update("timing", "asap")} className={cn("h-12 rounded-xl text-[11px] font-black", form.timing === "asap" ? "bg-primary text-white" : "bg-secondary")}>في أسرع وقت</button><button onClick={() => update("timing", "scheduled")} className={cn("h-12 rounded-xl text-[11px] font-black", form.timing === "scheduled" ? "bg-primary text-white" : "bg-secondary")}>موعد لاحق</button></div>{form.timing === "scheduled" && <div className="mt-3 grid grid-cols-2 gap-3"><Input type="date" value={form.date} min={new Date().toISOString().slice(0,10)} onChange={(event) => update("date", event.target.value)} className="h-12 rounded-xl" /><Input type="time" value={form.time} onChange={(event) => update("time", event.target.value)} className="h-12 rounded-xl" /></div>}</div></div>}
